// ignore_for_file: prefer_adjacent_string_concatenation

mixin StringConstants {
  static const fraazo = "Fraazo";
  static const nowFloats = "NowFloats";
  static const responsibility = "Responsibility";
  static const skills = "Skills";
  static const fraazoResponsibility =
      "- Following Agile methodology for Software Development Life Cycle.\n";
  static const nowFloatsResponsibility = "";
  static const instagramUrl = "https://www.instagram.com/it_is_nikhil/";
  static const linkedinUrl =
      "https://www.linkedin.com/in/nikhil-swami-005030210/";

  // Fraazo
  static const summary = "Summary";
  static const summaryDesc =
      "Designed, Implemented and tested flutter and dart code for both android and IOS.";
  static const rest = "REST APIs";
  static const restDesc =
      "Integrated APIs, Web Sockets and external dependencies within the application.";
  static const majorModulesFraazoDesc =
      "Major Modules include Server Driven UI, Order Tracking and Subscription.";
  static const firebaseFraazoDesc =
      "- Using Firebase for Push Notifications, dynamic links, Crashlytics and controlling variant distribution (A/B testing).";
  static const devTesting = "Dev Testing";
  static const devTestingDesc =
      "Worked on Bug fixes, unit and widget testing, state management and native features.";

  // NowFloats
  static const migration = "Migration";
  static const migrationDesc =
      "Migrating native android application (Zadinga) to flutter for android, Ios, web and desktop.";
  static const systemArchitecture = "System Architecture";
  static const systemArchitectureDesc =
      "Using Bloc along with MVVM architecture , proficiently configuring flavors and custom navigation.";
  static const firebase = "Firebase";
  static const firebaseDesc =
      "Utilized Firebase for Authentication, Push Notifications, Deep links, Analytics, and FireStore DB";
  static const localDatabase = "Local Database";
  static const localDatabaseDesc =
      "Used floor for local db and used mlkit_scanner for barcode and QR code scanning along with isolates for background tasks.";
  static const deployment = "Deployment";
  static const deploymentDesc =
      "Deployed application on play store and app store";
  static const maps = "Google Maps";
  static const mapsDesc =
      "Handled complex logic including search functionality, filtering, pagination on live observable data, video player, and google maps.";
  static const modifyingDependencies = "Modifying Dependencies";
  static const modifyingDependenciesDesc =
      "- Forked packages and plugins on gitHub and customized according to project's need.";
  static const majorModules = "Major modules";
  static const majorModulesDesc =
      "Major Modules include Order Management, Product Listing, Billing, Dashboard and Payments.";

  // Hero section (main page)
  static const heroLine1 = "Hello.";
  static const heroLine2 = "I'm Nikhil.";
  static const heroLine3 = "I build intelligent software.";
  static const heroCredentials = "Software Developer  •  ML Engineer";
  static const footerTitle = "Let's Connect";
  static const footerEmail = "nswami1@asu.edu";

  // Emulator onboarding
  static const emulatorHeadline =
      "Experience my portfolio.\nOn a smartphone.\nRight here.";
  static const emulatorPrompt = "Tap to explore";
}
